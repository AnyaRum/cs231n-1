...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        relu_layer = relu(first_layer)

        # Do the same thing for the second layer
        second_layer = np.dot(relu_layer, W2)
        second_layer += b2
        ...
        softmax_scores = np.sum(-1 * scores[range(N), y] + np.log(softmax_exp)) / N
        loss = softmax_scores
